...
float getLuxFromResistance(float R) {
  if (R >= R_ref[0]) return lux_ref[0];  // pretemno
  if (R <= R_ref[numPoints - 1]) return lux_ref[numPoints - 1];  // presvetlo
...
void loop() {
// izmeri povprečno vrednost na pol sekunde
  float R_total = 0;
  for (int i = 0; i < 5; i++) {
    R_total += readLDRResistance(); 
    delay(100); // 5 vzorcev se vzame 
  }
  R_LDR = R_total / 5.0;
  lcd.clear();
...
float readLDRResistance() {
  ...
  // Preveri, če je napetost previsoka in preklopi na 2. merilno območje
  if (voltage > 3.7) {
